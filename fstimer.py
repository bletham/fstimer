...
class PyTimer:
  ...
  def genwin1_2(self):
    ...
    self.win1_2.set_border_width(20)
    self.win1_2.set_size_request(600, 400)
    self.win1_2.connect('delete_event',lambda b,jnk: self.win1_2.hide())
    ##Now create the vbox.
    vbox1 = gtk.VBox(False,10)
    self.win1_2.add(vbox1)
    ##Now add the text.
    ...
    self.fields = ['Last name','First name','ID','Age','Gender','Address','Email','Telephone','Contact for future races','How did you hear about race'] #this is really just self.fieldsdic.keys(), but is important because it defines the order in which fields show up on the registration screen
    self.fieldsdic = {}
    ...
    for field in self.fields:
      if self.fieldsdic[field]['type'] == 'entrybox':
        self.regfieldsmodel.append([field,'entrybox','max characters: '+str(self.fieldsdic[field]['max'])])
    ...
    btnCANCEL = gtk.Button(stock=gtk.STOCK_CANCEL)
    btnCANCEL.connect('clicked',lambda btn: self.win1_2.hide())
    alignCANCEL = gtk.Alignment(0,0,0,0)
    alignCANCEL.add(btnCANCEL)
    btnBACK = gtk.Button(stock=gtk.STOCK_GO_BACK)
    btnBACK.connect('clicked',self.win1_2_back)
    btnNEXT = gtk.Button(stock=gtk.STOCK_GO_FORWARD)
    btnNEXT.connect('clicked',self.win1_2_next)
    ##And populate
  ...
  def regfield_up(self,jnk,selection):
  ...
  def regfield_down(self,jnk,selection):
    model,treeiter1 = selection.get_selected()
    if treeiter1:
      row = self.regfieldsmodel.get_path(treeiter1)
      row = row[0]
  ...
  def regfield_edit(self,jnk,selection):
    model,treeiter1 = selection.get_selected()
    if treeiter1:
  ...
  def regfield_new_entrybox(self,jnk,name,maxchar,treeiter):
    self.winnewentry = gtk.Window(gtk.WINDOW_TOPLEVEL)
    self.winnewentry.modify_bg(gtk.STATE_NORMAL, self.bgcolor)
    self.winnewentry.set_transient_for(self.win1_2)
    ...
    self.winnewentry.connect('delete_event',lambda b,jnk: self.winnewentry.hide())
  ...
  def regfield_new_combobox(self,jnk,name,options,treeiter):
    self.winnewcombo = gtk.Window(gtk.WINDOW_TOPLEVEL)
    self.winnewcombo.modify_bg(gtk.STATE_NORMAL, self.bgcolor)
    self.winnewcombo.set_transient_for(self.win1_2)
    ...
    self.winnewcombo.connect('delete_event',lambda b,jnk: self.winnewcombo.hide())
    label1 = gtk.Label('Field name:')
    nameentry = gtk.Entry(max=50)
    nameentry.set_text(name)
    hbox1 = gtk.HBox(False,10)
  ...
  def regfield_remove(self,jnk,selection):
    model,treeiter1 = selection.get_selected()
    if treeiter1:
      row = self.regfieldsmodel.get_path(treeiter1)
      row = row[0]
  ...
  def winnewcomboOK(self,jnk,treeiter,nameentry1,optionentry1,label3):
  ...
  def winnewentryOK(self,jnk,treeiter,nameentry1,maxchar1,label3):
